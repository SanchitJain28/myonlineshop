...
//logging in a user
router.post("/api/loginuser", body("email").notEmpty().withMessage("Email cannot be empty").isEmail().withMessage("Please Enter the correct email"),
...
//Seller account
router.post("/api/selleraccount", body("email").notEmpty().withMessage("email cannot be empty").isEmail().withMessage("enter a valid email"),
    body("password").notEmpty().withMessage("password cannot be empty"),
    body("businessName").notEmpty().withMessage("Business name cannot be empty be empty"),
    ...
    , async (req, res) => {
        try {
            ...
            const token = jwt.sign({ BusinessName: sellAccount.businessName, sellerid: sellAccount.id }, secret)

            res.send({ "Success": "Accound Succesfully created", seller: sellAccount, token: token })
        } catch (error) {
            console.log(error)
        }
...
//seller login
router.post("/api/sellerlogin", body('email').notEmpty().withMessage("Email cannot be empty").isEmail().withMessage("Enter a valid email"),
    body('password').notEmpty().withMessage("Password cannot be empty"), async (req, res) => {
        try {
            const result = validationResult(req);
            if (!result.isEmpty()) {
            ...
            if (!findSeller) {
                return res.send({ errors: [{ "msg": "Cant find you as a seller ,Please first register as a seller" }] })
            }
            const comparePassword = await bcrypt.compare(req.body.password, findSeller.password)
            if (!comparePassword) {
                return res.send({ errors: [{ "msg": "Password is Incorrect" }] })
            }
            const token = jwt.sign({ BusinessName: findSeller.businessName, sellerid: findSeller.id, email: findSeller.email }, secret)
...
router.get("/api/userdetails", verifyUser, async (req, res) => {
    try {
        ...
        const userOrders=await order.find({user:user_id}).populate("orderItems.product")
